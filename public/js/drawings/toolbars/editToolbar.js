...
import { loadEditToolbarPos, saveEditToolbarPos } from "./favoritesStore.js";
import { createTvMenu, LINE_WIDTH_MENU_ITEMS, LINE_STYLE_MENU_ITEMS } from "../ui/tvMenu.js";
import { createDrawingSettingsDialog } from "../ui/drawingSettingsDialog.js";
import { wireToolbarHints, hideToolbarHint } from "../ui/drawToolHint.js";

const EDIT_TOOL_HINTS = {
  templates: "Templates",
  "line-tool-color": "Color",
...
export function mountEditToolbar(opts) {
  ...
  widgets.innerHTML = `
    ...
    <button type="button" class="floating-toolbar-react-widgets__button" data-name="line-tool-color" aria-label="Color">
  ...
  function openWidthMenu(anchor) {
    ...
    tvMenu.open(anchor, LINE_WIDTH_MENU_ITEMS, {
      ...
      onSelect: (id) => {
        controller.updateDrawing(drawing.id, { lineWidth: Number(id) });
        syncFromDrawing();
      },
    });
  }
  ...
  controller.on("editText", (drawing) => {
    settingsDialog.open(drawing, { tab: "text" });
  });
