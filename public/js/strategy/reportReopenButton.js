...
/**
 * Bottom-bar control to reopen a dismissed strategy tester report.
 * @param {{ onReopen?: () => void }} [opts]
 */
export function createStrategyReportReopenButton(opts = {}) {
  ...
  function ensureMount() {
    ...
    if (!btn) {
      ...
      btn.type = "button";
      btn.className = "tv-strategy-report-reopen";
      btn.hidden = true;
  ...
  function sync(state) {
    ...
    el.title = `Open ${state.title} report`;
    el.setAttribute("aria-label", `Open strategy tester: ${state.title}`);
  }
