...
function stackedAxisLabelExtraHeight(rendererOptions) {
    return STACKED_LABEL_LINE_GAP + STACKED_SUBTITLE_FONT_SIZE + STACKED_LABEL_BOTTOM_PAD;
}
function standardAxisLabelHeight(rendererOptions) {
    return rendererOptions._internal_fontSize + rendererOptions._internal_paddingTop + rendererOptions._internal_paddingBottom;
}
function subtitleFont(rendererOptions) {
    return `${STACKED_SUBTITLE_FONT_SIZE}px ${rendererOptions._internal_fontFamily}`;
}
class PriceAxisViewRenderer {
    ...
    _internal_height(rendererOptions, useSecondLine) {
        ...
        const subtitle = this._private__data._internal_subtitleText ?? '';
        if (subtitle.length > 0 || useSecondLine) {
            return standardAxisLabelHeight(rendererOptions) + stackedAxisLabelExtraHeight();
        }
        return rendererOptions._internal_fontSize + rendererOptions._internal_paddingTop + rendererOptions._internal_paddingBottom;
    }
    _internal_draw(target, rendererOptions, textWidthCache, align) {
        ...
        if (subtitle.length > 0) {
            this._private__drawStackedLabel(target, rendererOptions, textWidthCache, align, subtitle);
            return;
        }
    ...
    _private__drawStackedLabel(target, rendererOptions, textWidthCache, align, subtitle) {
        const textColor = this._private__data._internal_color;
        const backgroundColor = this._private__commonData._internal_background;
        const lineY = this._private__commonData._internal_coordinate;
        ...
        const totalH = singleH + stackedAxisLabelExtraHeight();
        const alignRight = align === 'right';
        const layout = target.useBitmapCoordinateSpace((scope) => {
            const { context: ctx, horizontalPixelRatio, verticalPixelRatio } = scope;
            ctx.font = rendererOptions._internal_font;
            const priceWidth = Math.ceil(textWidthCache._internal_measureText(ctx, this._private__data._internal_text));
            ctx.font = subtitleFont(rendererOptions);
            const subtitleWidth = Math.ceil(textWidthCache._internal_measureText(ctx, subtitle));
            ctx.font = rendererOptions._internal_font;
            const geom = this._private__calculateGeometry(scope, rendererOptions, textWidthCache, align, {
                _internal_text: this._private__data._internal_text,
                _internal_textWidth: Math.max(priceWidth, subtitleWidth),
                _internal_coordinate: lineY,
            });
            const gb = geom._internal_bitmap;
            const blockTopBitmap = Math.round(topY * verticalPixelRatio);
            const blockHeightBitmap = Math.round(totalH * verticalPixelRatio);
            const blockXBitmap = alignRight ? gb._internal_xOutside : gb._internal_xInside;
            const cornerRadii = alignRight
                ? [gb._internal_radius, 0, 0, gb._internal_radius]
                : [0, gb._internal_radius, gb._internal_radius, 0];
            drawRoundRectWithBorder(ctx, blockXBitmap, blockTopBitmap, gb._internal_totalWidth, blockHeightBitmap, backgroundColor, gb._internal_horzBorder, cornerRadii, backgroundColor);
            if (this._private__data._internal_tickVisible) {
                ctx.fillStyle = textColor;
                ctx.fillRect(gb._internal_xInside, gb._internal_yMid, gb._internal_xTick - gb._internal_xInside, gb._internal_tickHeight);
            }
            if (this._private__data._internal_borderVisible) {
                ctx.fillStyle = rendererOptions._internal_paneBackgroundColor;
                ctx.fillRect(alignRight ? gb._internal_right - gb._internal_horzBorder : 0, blockTopBitmap, gb._internal_horzBorder, blockHeightBitmap);
            }
            const blockLeft = blockXBitmap / horizontalPixelRatio;
            const blockRight = blockLeft + gb._internal_totalWidth / horizontalPixelRatio;
            return {
                _internal_priceX: geom._internal_media._internal_xText,
                _internal_priceY: (geom._internal_media._internal_yTop + geom._internal_media._internal_yBottom) / 2 + geom._internal_media._internal_textMidCorrection,
                _internal_subtitleCenterX: (blockLeft + blockRight) / 2,
                _internal_subtitleTop: subtitleTop,
            };
        });
        target.useMediaCoordinateSpace(({ context: ctx }) => {
            ctx.fillStyle = textColor;
            ...
            ctx.textBaseline = 'middle';
            ctx.fillText(this._private__data._internal_text, layout._internal_priceX, layout._internal_priceY);
            ctx.textAlign = 'center';
            ctx.textBaseline = 'top';
            ctx.font = subtitleFont(rendererOptions);
            ctx.fillText(subtitle, layout._internal_subtitleCenterX, layout._internal_subtitleTop);
        });
    }
    // eslint-disable-next-line complexity
    _private__calculateGeometry(scope, rendererOptions, textWidthCache, align, overrides) {
        const { context: ctx, bitmapSize, mediaSize, horizontalPixelRatio, verticalPixelRatio } = scope;
        const tickVisible = overrides?._internal_tickVisible ?? (this._private__data._internal_tickVisible || !this._private__data._internal_moveTextToInvisibleTick);
        const tickSize = tickVisible ? rendererOptions._internal_tickLength : 0;
        const horzBorder = (overrides?._internal_separatorVisible ?? this._private__data._internal_separatorVisible) ? rendererOptions._internal_borderSize : 0;
        ...
        const text = overrides?._internal_text ?? this._private__data._internal_text;
        ...
        const textWidth = overrides?._internal_textWidth ?? Math.ceil(textWidthCache._internal_measureText(ctx, text));
        ...
        const yMid = overrides?._internal_coordinate ?? this._private__commonData._internal_fixedCoordinate ?? this._private__commonData._internal_renderCoordinate ?? this._private__commonData._internal_coordinate;
...
class CrosshairTimeAxisView {
    ...
    _private__resolveTimeScalePoint(timeScale) {
        const value = this._private__valueProvider();
        if (value !== null && Number.isFinite(value._internal_coordinate)) {
            const index = timeScale._internal_coordinateToIndex(value._internal_coordinate);
            const point = timeScale._internal_indexToTimeScalePoint(index);
            if (point !== null) {
                return point;
            }
        }
        return timeScale._internal_indexToTimeScalePoint(this._private__crosshair._internal_appliedIndex());
    }
    _private__updateImpl() {
        const data = this._private__rendererData;
        ...
        const currentTime = this._private__resolveTimeScalePoint(timeScale);
        if (currentTime === null) {
            return;
        }
...
class Crosshair extends DataSource {
    constructor(model, options) {
        ...
        const valueTimeProvider = (rawCoordinateProvider) => {
            return () => {
                const coordinate = rawCoordinateProvider();
                ...
                const timeScale = this._private__model._internal_timeScale();
                const index = timeScale._internal_coordinateToIndex(coordinate);
                const point = timeScale._internal_indexToTimeScalePoint(index);
                if (point === null) {
                    return null;
                }
        ...
        const currentPosTimeProvider = valueTimeProvider(() => this._internal_originCoordX());
    ...
    _private__tryToUpdateData(newIndex, newPrice, newPane) {
        ...
        const originX = this._internal_originCoordX();
        this._private__x = Number.isFinite(originX)
            ? originX
            : (isNaN(newIndex) ? NaN : this._private__model._internal_timeScale()._internal_indexToCoordinate(newIndex));
...
const priceLinePillSegmentDefaults = {
    text: '',
    backgroundColor: '',
    textColor: '#ffffff',
...
const priceLinePillCancelDefaults = {
    visible: true,
...
function measureTextWidth(ctx, text, weight, size, family) {
...
function hasShellBorder(bodyBorderColor) {
...
function layoutRow(ctx, data) {
    ...
    const rowLeft = pills.side === 'left'
        ? PILL_INSET + offset
        : Math.max(0, data._internal_paneWidth - totalW - offset);
    return { _internal_bodyText: rawBody, _internal_qtyText: qtyText, _internal_bodyW: bodyW, _internal_qtyW: qtyW, _internal_totalW: totalW, _internal_rowLeft: rowLeft };
}
function drawCancelIcon(ctx, cx, cy, color) {
    const half = 4;
...
class OrderPillsRenderer {
    ...
    _internal_updateY(y) {
        if (this._private__data !== null) {
            this._private__data._internal_y = y;
        }
    }
    _private__layoutCacheKeyFor(data) {
        const pills = data._internal_pills;
        const body = pills.body;
        const quantity = pills.quantity;
        return [
            data._internal_paneWidth,
            pills.side,
            pills.offset,
            body.visible,
            body.text,
    ...
    _private__resolveLayout(ctx, data) {
        const key = this._private__layoutCacheKeyFor(data);
        if (key === this._private__layoutCacheKey && this._private__cachedLayout !== null) {
            return this._private__cachedLayout;
        }
        const row = layoutRow(ctx, data);
        this._private__layoutCacheKey = key;
...
class CustomPriceLineOrderPillsPaneView {
    constructor(series, priceLine) {
        this._private__renderer = new OrderPillsRenderer();
        this._private__invalidated = true;
        this._private__layoutKey = '';
        this._private__series = series;
        this._private__priceLine = priceLine;
        this._private__rendererData = {
            _internal_y: 0,
            _internal_accentColor: '#089981',
            _internal_pills: priceLinePillsDefaults,
            _internal_paneWidth: 0,
            _internal_visible: false,
            _internal_isMoving: false,
        };
    }
    _internal_update() {
        this._private__invalidated = true;
    ...
    _internal_renderer() {
        ...
        return this._private__rendererData._internal_visible ? this._private__renderer : null;
    }
    _private__updateImpl() {
        const options = this._private__priceLine._internal_options();
        const data = this._private__rendererData;
        data._internal_visible = false;
        if (!this._private__priceLine._internal_isOrderLine() || !options._internal_pills?.visible) {
            return;
        }
        ...
        const pane = this._private__series._internal_model()._internal_paneForSource(this._private__series);
        const paneWidth = this._private__series._internal_model()._internal_timeScale()._internal_width() || pane?._internal_width() || 0;
        const pills = options._internal_pills ?? data._internal_pills;
        const layoutKey = [
            paneWidth,
            pills.side,
            pills.offset,
...
class CustomPriceLinePriceAxisView extends PriceAxisView {
    ...
    _internal__updateRendererData(axisRendererData, paneRendererData, commonData) {
        ...
        axisRendererData._internal_text = options.axisLabelText || this._private__formatPrice(options.price);
        axisRendererData._internal_subtitleText = options.axisSubtitleText || undefined;
        axisRendererData._internal_visible = true;
        ...
        commonData._internal_fixedCoordinate = undefined;
...
function syncOrderLineOptionKeys(target) {
    const rec = target;
    const pills = (rec.pills ?? rec._internal_pills);
    if (pills != null) {
        rec.pills = pills;
        rec._internal_pills = pills;
    }
    const order = (rec.order ?? rec._internal_order);
    if (order != null) {
        rec.order = order;
...
class CustomPriceLine {
    constructor(series, options) {
        ...
        this._private__orderPillsPaneView = new CustomPriceLineOrderPillsPaneView(series, this);
    ...
    _internal_isOrderLine() {
        return Boolean(this._private__options._internal_order);
    }
    _internal_paneView() {
        return this._private__priceLineView;
    }
    _internal_orderPillsPaneView() {
        return this._private__orderPillsPaneView;
    }
...
class PlotList {
    ...
    _internal_updateLastRow(plotRow) {
        const len = this._private__items.length;
        if (len === 0 || this._private__items[len - 1]._internal_index !== plotRow._internal_index) {
            return false;
        }
        this._private__items[len - 1] = plotRow;
        this._private__minMaxCache.clear();
        return true;
    }
    _internal_prependRows(plotRows) {
        ...
        this._private__indices = plotRows.map((plotRow) => plotRow._internal_index).concat(this._private__indices);
    }
...
class Series extends PriceDataSource {
     ...
     */
    _internal_updateLastBar(data, updateInfo) {
        if (data.length === 0 || this._private__data._internal_size() !== data.length) {
            return false;
        }
        const lastRow = data[data.length - 1];
        const prevLast = this._private__data._internal_last();
        if (prevLast === null || prevLast._internal_index !== lastRow._internal_index) {
            return false;
        }
        if (!this._private__data._internal_updateLastRow(lastRow)) {
            return false;
        }
        if (this._internal_isConflationEnabled() && isSeriesPlotRow(lastRow)) {
            this._internal_updateLastConflatedChunk(lastRow);
        }
        const paneView = this._private__paneView;
        if (typeof paneView._internal_patchLastRawPoint === 'function' && paneView._internal_patchLastRawPoint(lastRow)) ;
        else {
            paneView._internal_update('data');
        }
        if (this._private__lastPriceAnimationPaneView !== null && updateInfo?._internal_lastBarUpdatedOrNewBarsAddedToTheRight) {
            this._private__lastPriceAnimationPaneView._internal_onNewRealtimeDataReceived();
        }
        const sourcePane = this._internal_model()._internal_paneForSource(this);
        this._internal_model()._internal_recalculatePane(sourcePane);
    ...
    _internal_prependData(data, updateInfo) {
        if (data.length === 0) {
            return;
        }
        this._private__data._internal_prependRows(data);
        this._private__conflationByFactorCache.clear();
        this._private__paneView._internal_update('data');
        const sourcePane = this._internal_model()._internal_paneForSource(this);
        this._internal_model()._internal_recalculatePane(sourcePane);
    ...
    _internal_createOrderLine(options) {
        const result = new CustomPriceLine(this, {
            ...options,
            axisSubtitleText: '',
            _internal_order: true,
        });
        this._private__customPriceLines.push(result);
        this._internal_model()._internal_updateSource(this);
        return result;
    }
    _internal_removeOrderLine(line) {
        this._internal_removePriceLine(line);
    }
    _internal_orderLines() {
        return this._private__customPriceLines.filter((line) => line._internal_isOrderLine());
    }
    ...
    _internal_paneViews() {
        ...
        const priceLineViews = this._private__customPriceLines
            .filter((line) => !line._internal_isOrderLine())
            .map((line) => line._internal_paneView());
...
class TimeScale {
    constructor(model, options, localizationOptions, horzScaleBehavior) {
        ...
        this._private__scrollStartPoint = null;
        this._private__suppressLogicalRangeChanged = false;
        this._private__logicalRangeChangedWhileSuppressed = false;
        this._private__scaleStartPoint = null;
    ...
    _internal_indexToTime(index) {
        const point = this._internal_indexToTimeScalePoint(index);
        return point?.time ?? null;
    }
    _internal_indexToTimeScalePoint(index) {
        const point = this._private__points[index];
        if (point !== undefined) {
            return point;
        }
        return this._private__extrapolatedTimeScalePoint(index);
    }
    _internal_timeToIndex(time, findNearest) {
        ...
        const timeKey = this._private__horzScaleBehavior.key(time);
        const lastIdx = (this._private__points.length - 1);
        const lastKey = this._private__horzScaleBehavior.key(this._private__points[lastIdx].time);
        const firstKey = this._private__horzScaleBehavior.key(this._private__points[0].time);
        const interval = this._private__barInterval();
        if (timeKey > lastKey) {
            if (!findNearest) {
                return null;
            }
            ...
            return (lastIdx + Math.max(1, shift));
        }
        if (timeKey < firstKey) {
            if (!findNearest) {
                return null;
            ...
            return (0 - Math.max(1, shift));
        }
        const index = lowerBound(this._private__points, timeKey, (a, b) => this._private__horzScaleBehavior.key(a.time) < b);
    ...
    _internal_timeRangeForLogicalRange(range) {
        ...
        return {
            ...
            to: ensureNotNull(this._internal_indexToTimeScalePoint(to)),
        };
    }
    ...
    _internal_marks() {
        ...
        const virtualItems = this._private__buildVirtualTickMarks(firstBar, lastBar, maxIndexesPerMark, showAllMarks, items);
        const allItems = items.concat(virtualItems).sort((a, b) => a.index - b.index);
    ...
    _internal_startScroll(x) {
        ...
        this._private__scrollStartPoint = x;
        this._private__saveCommonTransitionsStartState();
        this._private__suppressLogicalRangeChanged = true;
    }
    _internal_scrollTo(x) {
        if (this._private__scrollStartPoint === null) {
            return;
    ...
    _internal_endScroll() {
        ...
        if (this._private__visibleRangeInvalidated) {
            ...
            this._private__flushSuppressedLogicalRangeChanged();
        }
    }
    ...
    _private__barInterval() {
        if (this._private__points.length < 2) {
            return null;
        }
        const prev = this._private__points[this._private__points.length - 2];
        const last = this._private__points[this._private__points.length - 1];
        const delta = this._private__horzScaleBehavior.key(last.time) - this._private__horzScaleBehavior.key(prev.time);
     ...
     */
    _private__extrapolatedTimeScalePoint(index) {
        if (this._private__points.length === 0) {
            return null;
        }
        const lastIdx = this._private__points.length - 1;
        const anchorIdx = index > lastIdx ? lastIdx : 0;
        const anchor = this._private__points[anchorIdx];
        const interval = this._private__barInterval();
        if (interval === null) {
            return null;
        }
        ...
        const anchorKey = this._private__horzScaleBehavior.key(anchor.time);
        const virtualKey = anchorKey + shift * intervalKey;
        const anchorOriginal = anchor.originalTime;
        let originalTime;
        if (typeof anchorOriginal === 'number') {
            originalTime = (anchorOriginal + shift * intervalKey);
        }
        else if (typeof anchorOriginal === 'object' &&
            anchorOriginal !== null &&
            typeof anchorOriginal._internal_timestamp === 'number') {
            const anchorTs = anchorOriginal;
            const shiftedTimestamp = (anchorTs._internal_timestamp + shift * intervalKey);
            // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
            originalTime = { _internal_timestamp: shiftedTimestamp };
        }
        else {
            originalTime = virtualKey;
        }
        const anchorTp = anchor.time;
        if (anchorTp._internal_timestamp !== undefined && anchorTp._internal_businessDay === undefined) {
            return {
                timeWeight: anchor.timeWeight,
                time: { _internal_timestamp: virtualKey },
                originalTime,
            };
        }
        return {
            timeWeight: anchor.timeWeight,
            time: this._private__horzScaleBehavior.convertHorzItemToInternal(originalTime),
            originalTime,
        };
    }
    ...
    _private__flushSuppressedLogicalRangeChanged() {
        if (!this._private__logicalRangeChangedWhileSuppressed) {
            return;
        }
        this._private__logicalRangeChangedWhileSuppressed = false;
    ...
    _private__internalTimeToDate(time) {
        const tp = time;
        if (tp._internal_businessDay !== undefined) {
            const bd = tp._internal_businessDay;
            return new Date(Date.UTC(bd.year, bd.month - 1, bd.day));
        }
        return new Date(tp._internal_timestamp * 1000);
    }
    /**
     * Tick marks for virtual indices beyond loaded data (future / past whitespace).
     */
    _private__buildVirtualTickMarks(firstBar, lastBar, maxIndexesPerMark, showAll, realItems) {
        const lastIdx = this._private__lastIndex();
        const firstIdx = this._private__firstIndex();
        if (lastIdx === null || firstIdx === null || this._private__points.length === 0) {
            return [];
        }
        const lastRealIndex = lastIdx;
        const firstRealIndex = firstIdx;
        const rangeStart = firstBar;
        const rangeEnd = lastBar;
        ...
        const addVirtualIndex = (index) => {
            ...
            const prevPoint = this._internal_indexToTimeScalePoint((index - 1));
            if (prevPoint === null) {
                return;
            }
            ...
            const weight = weightByTime(this._private__internalTimeToDate(point.time), this._private__internalTimeToDate(prevPoint.time));
            marks.push({
                index: index,
                time: point.time,
                weight,
    ...
    _private__setVisibleRange(newVisibleRange) {
        ...
        if (!areRangesEqual(oldVisibleRange._internal_logicalRange(), this._private__visibleRange._internal_logicalRange())) {
            if (this._private__suppressLogicalRangeChanged) {
                this._private__logicalRangeChangedWhileSuppressed = true;
            }
            else {
                this._private__logicalRangeChanged._internal_fire();
            }
...
class FastTimezoneProvider {
    constructor(zoneName, transitions) {
        this._private__zoneName = zoneName;
        this._private__transitions = transitions;
    }
    zoneName() {
        return this._private__zoneName;
    }
    getOffset(utcTimestamp) {
        const transitions = this._private__transitions;
        if (transitions.length === 0) {
            return 0;
        }
        const index = lowerBound(transitions, utcTimestamp, (t, ts) => t.utcTimestamp <= ts) - 1;
 ...
 */
function shiftUtcTimestampForDisplay(utcTimestamp, provider) {
 ...
 */
function createFastTimezoneProvider(zoneName, transitions) {
...
class HorzScaleBehaviorTime {
    ...
    formatHorzItem(item) {
        const tp = item;
        const timestamp = this._private__displayTimestamp(tp);
        return this._private__dateTimeFormatter._internal_format(new Date(timestamp * 1000));
    }
    formatTickmark(tickMark, localizationOptions) {
        ...
        const timePoint = tickMark.time;
        return defaultTickMarkFormatter(this._private__displayTimePoint(timePoint), tickMarkType, localizationOptions.locale);
    }
    _private__displayTimestamp(timePoint) {
        if (timePoint._internal_businessDay !== undefined) {
            return timePoint._internal_timestamp;
        }
        return shiftUtcTimestampForDisplay(timePoint._internal_timestamp, this._private__options.timezoneProvider);
    }
    _private__displayTimePoint(timePoint) {
        if (timePoint._internal_businessDay !== undefined) {
            return timePoint;
        }
        const timestamp = shiftUtcTimestampForDisplay(timePoint._internal_timestamp, this._private__options.timezoneProvider);
        if (timestamp === timePoint._internal_timestamp) {
            return timePoint;
        }
        return { ...timePoint, _internal_timestamp: timestamp };
    }
...
class DataLayer {
    ...
    // eslint-disable-next-line complexity
    _internal_prependSeriesData(series, data) {
        if (data.length === 0) {
            return this._private__emptyUpdateResponse();
        }
        if (series._internal_isConflationEnabled()) {
        ...
        const existingRows = this._private__seriesRowsBySeries.get(series);
        if (existingRows === undefined || existingRows.length === 0) {
            return this._internal_setSeriesData(series, data);
        }
        const firstExistingKey = this._private__horzScaleBehavior.key(existingRows[0]._internal_time);
        const originalTimes = data.map((d) => d.time);
        const timeConverter = this._private__horzScaleBehavior.createConverterToInternalObj(data);
        const createPlotRow = getSeriesPlotRowCreator(series._internal_seriesType());
        const dataToPlotRow = series._internal_customSeriesPlotValuesBuilder();
        ...
        for (let index = 0; index < data.length; ++index) {
            ...
            if (horzItemKey === firstExistingKey) {
                const pointDataAtTime = this._private__pointDataByTimePoint.get(horzItemKey);
                if (pointDataAtTime !== undefined) {
                    const plotRow = createPlotRow(time, pointDataAtTime._internal_index, item, originalTimes[index], dataToPlotRow, customWhitespaceChecker);
                    pointDataAtTime._internal_mapping.set(series, plotRow);
                    if (isSeriesPlotRow(plotRow)) {
                        existingRows[0] = plotRow;
                    }
                }
                continue;
            }
            let pointDataAtTime = this._private__pointDataByTimePoint.get(horzItemKey);
            const affectsTimeScale = pointDataAtTime === undefined;
            if (pointDataAtTime === undefined) {
                pointDataAtTime = createEmptyTimePointData(time);
                this._private__pointDataByTimePoint.set(horzItemKey, pointDataAtTime);
            }
            const plotRow = createPlotRow(time, pointDataAtTime._internal_index, item, originalTimes[index], dataToPlotRow, customWhitespaceChecker);
            pointDataAtTime._internal_mapping.set(series, plotRow);
            if (isSeriesPlotRow(plotRow)) {
                rowsToPrepend.push(plotRow);
            }
            if (affectsTimeScale) {
                const newPoint = {
                    timeWeight: 0,
                ...
                const insertIndex = lowerBound(this._private__sortedTimePoints, horzItemKey, (a, b) => this._private__horzScaleBehavior.key(a.time) < b);
                ...
                for (let pointIndex = insertIndex; pointIndex < this._private__sortedTimePoints.length; ++pointIndex) {
                    assignIndexToPointData(this._private__sortedTimePoints[pointIndex].pointData, pointIndex);
                }
            }
        }
        ...
        if (firstChangedPointIndex !== -1) {
            this._private__horzScaleBehavior.fillWeightsForPoints(this._private__sortedTimePoints, firstChangedPointIndex);
        }
        const info = {
            _internal_historicalUpdate: true,
            _internal_lastBarUpdatedOrNewBarsAddedToTheRight: false,
        };
        const response = this._private__getUpdateResponse(series, firstChangedPointIndex, info);
        if (rowsToPrepend.length > 0) {
            const seriesChanges = response._internal_series.get(series);
            if (seriesChanges !== undefined) {
                response._internal_series.set(series, { ...seriesChanges, _internal_prependRows: rowsToPrepend });
            }
        }
        return response;
    }
...
class SeriesPaneViewBase {
    ...
    _internal_patchLastRawPoint(row) {
        return false;
    }
    _internal_renderer() {
        if (!this._internal__series._internal_visible()) {
    ...
    attachPrimitive(primitive) {
        ...
        if (primitive.attached) {
            primitive.attached({
                ...
                requestUpdate: () => this._private__pane._internal_model()._internal_invalidatePane(this._private__pane),
            });
        }
    }
...
function checkOrderLineOptions(options) {
    assert(typeof options.price === 'number', `the type of 'price' order line's property must be a number, got '${typeof options.price}'`);
}
...
const orderLineOptionsDefaults = {
...
function toOrderLineOptions(options) {
    const rec = options;
    // eslint-disable-next-line @typescript-eslint/naming-convention
    const { _internal_order: _order, _internal_pills: _pills, axisSubtitleText: _subtitle, ...rest } = options;
    const pills = (rec.pills ?? rec._internal_pills ?? orderLineOptionsDefaults.pills);
    return {
        ...rest,
        pills,
    };
...
class OrderLine {
    constructor(orderLine) {
        this._private__orderLine = orderLine;
    }
    applyOptions(options) {
        this._private__orderLine._internal_applyOptions({ ...options, _internal_order: true });
    }
    options() {
        return toOrderLineOptions(this._private__orderLine._internal_options());
    }
    _internal_orderLine() {
        return this._private__orderLine;
    }
}
...
class SeriesApi {
    ...
    prepend(data) {
        ...
        checkItemsAreOrdered(data, this._private__horzScaleBehavior);
        checkSeriesValuesType(this._internal__series._internal_seriesType(), data);
        this._internal__dataUpdatesConsumer._internal_prependData(this._internal__series, data);
    ...
    priceLines() {
        return this._internal__series._internal_priceLines().filter((line) => !line._internal_isOrderLine()).map((priceLine) => new PriceLine(priceLine));
    }
    createOrderLine(options) {
        checkOrderLineOptions(options);
        const strictOptions = merge(clone(orderLineOptionsDefaults), options);
        const orderLine = this._internal__series._internal_createOrderLine(strictOptions);
        return new OrderLine(orderLine);
    }
    removeOrderLine(line) {
        this._internal__series._internal_removeOrderLine(line._internal_orderLine());
    }
    orderLines() {
        return this._internal__series._internal_orderLines().map((orderLine) => new OrderLine(orderLine));
    }
    seriesType() {
    ...
    attachPrimitive(primitive) {
        ...
        if (primitive.attached) {
            primitive.attached({
                chart: this._internal__chartApi,
                series: this,
                requestUpdate: () => this._internal__series._internal_model()._internal_updateSource(this._internal__series),
    ...
    detachPrimitive(primitive) {
        ...
        this._internal__series._internal_model()._internal_updateSource(this._internal__series);
    }
    getPane() {
...
class ChartApi {
    ...
    _internal_prependData(series, data) {
        this._private__sendUpdateToChart(this._private__dataLayer._internal_prependSeriesData(series, data));
    }
    ...
    _private__sendUpdateToChart(update) {
        const model = this._private__chartWidget._internal_model();
        const incrementalTimeScale = update._internal_timeScale._internal_firstChangedPointIndex === undefined;
        ...
        update._internal_series.forEach((value, series) => {
            if (value._internal_prependRows !== undefined && value._internal_prependRows.length > 0) {
                series._internal_prependData(value._internal_prependRows, value._internal_info);
                needsFullPaneRecalc = true;
            }
            else if (incrementalTimeScale &&
                value._internal_info?._internal_lastBarUpdatedOrNewBarsAddedToTheRight &&
                series._internal_updateLastBar(value._internal_data, value._internal_info)) ;
            else {
                series._internal_setData(value._internal_data, value._internal_info);
                needsFullPaneRecalc = true;
            }
        });
...
class LinePaneViewBase extends SeriesPaneViewBase {
    ...
    _internal_patchLastRawPoint(row) {
        if (this._internal__dataInvalidated || this._internal__items.length === 0) {
            return false;
        }
        const colorer = this._internal__series._internal_barColorer();
        ...
        this._internal__items[this._internal__items.length - 1] = this._internal__createRawItem(row._internal_index, value, colorer);
        this._internal__invalidated = true;
        return true;
    }
...
class BarsPaneViewBase extends SeriesPaneViewBase {
    ...
    _internal_patchLastRawPoint(row) {
        ...
        const colorer = this._internal__series._internal_barColorer();
        this._internal__items[this._internal__items.length - 1] = this._internal__createRawItem(row._internal_index, row, colorer);
        this._internal__invalidated = true;
        return true;
    }
    _internal__hitTestImpl(x, y) {
